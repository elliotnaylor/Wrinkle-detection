...
//Returns a value between 1 and 100 depending on specular reflection amount
int SpecularHighlightRemoval::specularAmount() {
    int spec = 0, skin = 0;
    float amount;

    for(int pixel = 0; pixel < specularImage.rows * specularImage.cols; pixel++) {
            if(diffuseImage.ptr<unsigned char>()[pixel * 3] != 0 &&
                    specularImage.ptr<unsigned char>()[pixel * 3] > 180) { //estimated value
                spec++;
            } else if(diffuseImage.ptr<unsigned char>()[pixel * 3] != 0) {
                skin++;
            }
    }

    //LOG("Returning specular amount");

    //scales from 0 to 100 on how much of the face has a specular reflection
    amount = spec / (spec + skin);
    specAmount = amount * 100;
    return specAmount;
